...
\section{Model interpretability}
	...
	defensive methods mainly developed on images, heuristics to identify adv example more geared towards an inference settings. not applicable in our case where we want to detect CFs
	most effective way is adversarial training. 
	...
	\end{equation}
	implemented as an alternating procedure: solve the inner maximization problem (find adversarial examples) with fixed model weigths \(\theta\)
	in a second step updates model weights: solve outer minimization problem on the computed adversarial examples.\footnote{In practice, it is more common to also include the standard training loss, \ie{}include clean examples along the adversarial ones.}
