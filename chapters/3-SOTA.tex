...
\section{Model interpretability}
	...
	Attribution methods aims at assigning scores to input features highlighting their impact on the model's output.
	many methods:gradient based (integrated gradients, grad cam PathCNN ), lrp, deeplift shap lime sensitivity (perturbation)
	using gradients, what represent gradients ? guided backprop --> Gradcam
	saturating gradients --> deep lift reference inputs

	backpropagate attribution signal decomposition of the signal LRP

	LIME/SHAP
	% add reference of papers using this in the precision medicine field. 
