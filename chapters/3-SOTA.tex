...
\section{Model interpretability}
	...
	Currently, there is no consensus on the terms interpretability or explainability; the definition often varies and depends on the context of use or the researcher.
	...
	Self-explainable methods use models that are interpretable by design, \ie{}able to provide a prediciton and a supporting interpretation.
	...
	The model is composed of two branches, a concept encoder and a relevance parametrizer.
	The concept encoder is used to map the raw input features to a set of interpretable concepts and the parametrizer learns the importance associated to each concept.
	Those importance coefficients are used to aggregate the concepts, as a linear combination into a prediction.
	Aggregating concepts with linear combination ensure interpretability of the importance coefficients.
	...
	All the paired score can be visualized through an attention map to see how each feature attends to other features.
	...
	\citeauthor{jain-wallace-2019-attention} showed that high attention weights does not necessarily correlate with attribution scores~\cite{jain-wallace-2019-attention}.
	\citeauthor{wiegreffe-pinter-2019-attention} highlihgted the importance of the definition of explainability to consider the attention mechanism as interpretable or not~\citeauthor{wiegreffe-pinter-2019-attention}.
	...
	Prototypes based approaches return a set of instances from the training set supporting the instance to interpret.
	Attribution methods aims at assigning scores to input features (or individual neurons) highlighting their impact on the model's output.
	As gradients of the loss function with respect to the input represents the impact of a small change in the input on the loss, they can be used as a proxy to estimate feature importance.
	This idea have been used to create saliency maps highlighting important parts of the inputs~\cite{intGrad}.
	Grad-CAM is another gradient-based approach designed to interpret images, the gradienu is backpropagated only up to the last convolutional layer~\cite{GradCAM}.
	...
	To circumvent this limitation, DeepLIFT proposed to use a baseline, the explanation is expressed relative to the choosen reference~\cite{DeepLIFT}.
	...
	Enrichment analysis was then used to check for biological relevance of the identified important features.
	...
	Such appraoch are costly as for each instance being explained a new local model needs to be constructed.
	SHAP~\cite{SHAP} is a method estimating feature importance based on Shapley values from game theory.
	The Shapley value of a feature represents its contribution to the prediciton, and the contribution can be interpreted as the importance of this feature.
	A version of SHAP specifically designed for neural networks, DeepSHAP~\cite{DeepSHAP}, has been used in different works to identify important pathways and modality contributions~\cite{Liu2024}, the most important genes across multiple omics~\cite{Withnell2021,customics}, to identify most important CpGs site~\cite{levyMethylNetAutomatedModular2020a}.
