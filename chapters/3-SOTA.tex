...
\section{Prediction from multi-omics data}
	...
	\subsection{Deep learning models for multi-omics data}
		...
		\subsubsection{Late fusion}
			...
			HyperTMO brca subtype
			mRNA miRNA DNAm
			hypergraphs construction which can better represent the complex association and heterogeneous of multi-omics data.
			each vertex corresponds to a patient sample, and
			each hyperedge contains an arbitrary subset of the set V
			The KNN algorithm (Zhang and Zhou 2007) was utilized
			to construct the hypergraph structure for each omics data
			type based on the sample similarity matrix. HGCN
			Evidence results of each omics data type are subjected to confidence and uncertainty estimation.
			Evidence from different views is combined to produce trusted
			results

			% 		The shortcoming of such integration strategy is that it cannot
			% capture inter-omics interactions and at no point in the learning
			% process can the different machine learning models share knowledge and utilize the complementarity information between omics.
			% Combining predictions is simply not enough to accurately exploit
			% multi-omics data and understand the underlying biological mechanisms of diseases. For that reason, and because it boils down to
			% multiple single-omics analysis, we will not discuss it further. \cite{Picard2021}

		\subsubsection{Intermediate fusion}
			Due to its flexibility... widely used
