...
\section{Methods}
	...
	\subsection{Getting a robust model: adversarial training}
		...
		The classifier can be made robust to adversarial examples with adversarial training~\cite{AdvTrainingMinMax}, which is achieved by solving the following min-max optimization problem:
		\begin{equation}
	...
	\subsection{Adversarial Metrics}
		To evaluate model robustness, we used three metrics: model smoothness~\cite{MarginDecision}, margin decision~\cite{MarginDecision} and boundary thickness~\cite{AdvBoundaryThick}.
		...
		\begin{description}[
			...
			\item[Model smoothness]
				...
				This metric is defined as the Kullback-Leibler divergence between the predicted probability of a clean sample \(g\left(\symbf{x}\right)\) and the predicted probability of an adversarial example \(g\left(\symbf{x}^{\star}\right)\):
				\begin{equation}
					\operatorname{KL}\left(g\left(\symbf{x}\right) \| g\left(\symbf{x}^{\star}\right)\right)
				\end{equation}
			...
			\item[Margin decision]
				...
				This is measured as the difference between the probability of the true label \(g\left(\symbf{x}\right)_{y}\) and the probability of the other most probable class \(\max_{i \neq y} g\left(\symbf{x}\right)_{i}\):
				\begin{equation}
					g\left(\symbf{x}\right)_{y} - \max_{i \neq y} g\left(\symbf{x}\right)_{i}
				\end{equation}
				A higher margin decision is better.
			\item[Boundary thickness]
				The boundaries thickness measure ~\cite{AdvBoundaryThick}.
				\begin{equation}
