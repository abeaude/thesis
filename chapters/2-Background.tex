...
\section{Deep-Learning models}
	...
	\subsection{Model training and evaluation}
		...
		% TODO improve survival introduction
	...
	\subsection{\Glsfmtfull{gan}}
		...
		\(G\) and \(D\) are jointly trained at generating a distribution \(p_{g}\) such that \(D\) cannot distinguish between \(p_{d}\) and \(p_{g}\).
		A \gls{gan} is trained by solving the following min-max problem:
		\begin{multline}
			\min_{G} \max_{D} \symcal{L}_{GAN}\left(G,D\right) = \\ \E_{x\sim p_{d}}\left[ \log\left(D\left(x\right)\right)\right] + \E_{z\sim p_{z}}\left[ \log\left( 1 - D\left(G\left(z\right)\right)\right)\right]
		\end{multline}
		...
		The complete training procedure is presented in XX. 

		Classical GAN problem Gradient vanishing problem.
		Risk of blocking in the early stages of
		GAN learning, when the discriminator
		work is very easy. mode collapse ? 
		What is the best way to measure the distance between two distributions ? Earth mover distance Wasserstein distance
		present earth mover distance 
		% https://arxiv.org/pdf/1904.08994
		% https://cloud.univ-grenoble-alpes.fr/index.php/s/wxCztjYBbQ6zwd6?dir=undefined&openfile=837709648
		% https://d2l.ai/chapter_generative-adversarial-networks/gan.html
		WGAN
		According to the authors, the discriminator should
no longer be considered as a classifier but more as a critic.
The output of the discriminator will be greater for true
instances than for false instances.
this approach has also some constraints, hard to satisfy 
