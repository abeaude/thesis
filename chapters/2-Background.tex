...
\section{Deep-Learning models}
	...
	\subsection{\Glsfmtfull{ae} and \Glsfmtfull{vae}}
		...
		The \gls{vae} adds regularization on the latent space by constraining the latent representation to follow a multivariate gaussian distribution, and each latent feature is represented by a gaussian distribution of parameters \(\mu\) and \(\sigma\)~\cite{VAEref}.
	...
	\subsection{Model training and evaluation}
		...
		\begin{mybox}[label={box:cox}]{Cox models}
			The Cox proportional hazards model\footnote{The hazard ratio of two patients is independant of time: \(\frac{\lambda(t | \symbf{x}_{i})}{\lambda(t | \symbf{x}_{j})} = e^{\beta\cdot \left(\symbf{x}_{i} - \symbf{x}_{j}\right)}\)} is a method used to estimate a patient survival from its covariates \(\symbf{x}\).
	...
	\subsection{\Glsfmtfull{gan}}\label{sec:gan}
		...
		The discriminator is now called a critic that instead of classifying samples as real or fake provide a realness score\footnote{There are no more sigmoid activation at its output}.
...
\section{Omics data}
	...
	\subsection{Transcriptomics}\label{subsec:transcriptomics}
		...
		The resulting count matrix can later be normalized to account for the sequencing depth and the length of the gene in \gls{fpkm}, \gls{tpm}, or any other normalization depending on the study needs.
	...
	\subsection{Proteomics}\label{subsec:proteomics}
		...
		Peptides mass-to-charge ratios are compared to theoritical ratios databases to identify the proteins.
