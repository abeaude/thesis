...
\section{Deep-Learning models}
	...
	\subsection{\Glsfmtfull{mlp}}\label{subsec:mlp}
		\begin{wrapfigure}[11]{O}{0.4\textwidth}
			\vspace{-1\intextsep}
			\centering
	...
	\subsection{\Glsfmtfull{gan}}\label{sec:gan}
		...
		The generator captures the data distribution and can generate new data \(\hat{x} = G\left(z\right)\), where \(z\) is a latent noise vector sampled from a prior distribution \(p_z\).
...
\section{Omics data}
	...
	\subsection{Epigenomics}
		...
		% \begin{figure}[htbp]
		% 	\centering
		% 	\includegraphics[scale=0.7]{epigeneticmechanisms.pdf}
		% 	\caption[Epigenetic mechanism]{Epigenetic mechanism, adapted from~\cite{IllustrationHowEpigenetic}}\label{fig:epigenetics}
		% \end{figure}
