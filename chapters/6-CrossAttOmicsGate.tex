...
\section{Method}
	...
	\subsection{Modality encoders}
	  Modalities are encoded with attention\-/based encoders,\(\enc^{i}\)~\cite{AttOmics}, similarly to \nameref{chap:crossattomics} described in~\cref{chap:crossattomics}.
	  ...
	  Each modality \(i\) is represented in a lower dimensional space: \(U^{i} = \enc^{i}\left(X^{i}\right)\). %preserve the group structure
	...
	\subsection{CrossAttention: modalities interactions}
	  ...
	  % contrary to CrossAttOmics we consider all possible interaction pairs (complete graph)
	...
	\subsection{Impact of the sparsity rate}
	  ...
	  \begin{figure}[htbp]
	    ...
	    \begin{subtable}[c]{0.33\textwidth}
	      ...
	      \end{tblr}
	    \end{subtable}
	    \caption[Impact of the sparsity rate \(\tau\)]{Impact of the sparsity rate \(\tau\) on (a) the number of selected interactions and (b) the accuracy.}
	  \end{figure}
