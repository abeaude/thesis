...
\begin{spacing}{1}
    \begin{tikzpicture}
    \node (a_lgd) [inner sep=0pt] at (-2,0.2) {\textbf{A}};
    \node (b_lgd) [right=14.7cm of a_lgd.east, anchor=west, inner sep=0pt] {\textbf{B}};
    \node (c_lgd) [below=5.2cm of a_lgd.south, anchor=north, inner sep=0pt] {\textbf{C}};
    \node (d_lgd) [right=8.5cm of c_lgd.east, anchor=west, inner sep=0pt ] {\textbf{D}};
    ...
    \foreach \lab/\c in {A/mod1, B/mod2, C/mod3}
        {
            \node (enc_\lab) [draw=\c, text=\c, thick, rounded rectangle, rounded rectangle west arc=none, right=5mm of in_\lab.east, anchor=west] {$\enc^{\lab}$};
            \node [text=\c, right=5mm of enc_\lab.north east, anchor=north west, inner sep=0pt, yshift=1mm] {\(U^{\lab}\)};
    ...
    \begin{scope}[xshift=7.5cm, yshift=-6.2cm, scale=0.9, transform shape, every node/.style={font=\tiny, thick}]
    ...
    \node (A) [inner sep=1pt, right=1cm of Z_A.east, anchor=west, text=mod1, yshift=2mm] {A};
