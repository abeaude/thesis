\begin{spacing}{1}
	...
	\begin{tikzpicture}<disable externalization>
		...
		\node[circle, fill=Prune, text=white, below=3cm of n4.south] (n5){\ref*{chap:crossattomicsgate}};
		...
		\node[right=1cm of n1.east, anchor=west] (b1) {
			\tcbox[title=Background]{
				In this chapter, the different deep learning architectures used during this thesis work are presented. A brief presentation of gene expression regulation and cancer formation is provided. The methods to obtain the different omics are described. Finally, the different datasets used are described.
			}
		};

		\node[left=1cm of n2.west, anchor=east] (b2) {
			\tcbox[title=State-of-the-art]{
				This chapter is dedicated to the description of the current deep learning methods used to predict a phenotype from omics data. A bries introduction to multimodal machine learning is provided before describing its application to integrate multi-omics data.
			}
		...
		\node[right=1cm of n3.east, anchor=west] (b3) {
			\tcbox[title=AttOmics]{
				...
				Groups are individually projected to compute intra-group interactions.
				\Glsfmtshort{mhsa} is then applied to incorporate inter-group interactions in the representation.
		...
		\node[right=1cm of n5.east, anchor=west] (b5) {
			\tcbox[title=CrossAttOmicsGate]{
				This chapter present a variation of the CrossAttOmics architecture. Instead of focusing on known omics interactions, cross attention is used on all pairs of omics. Then interactions are scored for each patient.
		...
		\node[left=1cm of n6.west, anchor=east] (b6) {
			\tcbox[title=Counterfactuals]{
				This chapter presents how \glsfmtshortpl{gan} can be used to find counterfactuals, the minimal perturbation changing the prediction, on omics data.
				The proposed perturbation can be linked to known drugs to identify potential treatments.
				The proposed changes can also help in the identification of biomarkers.
			}
		};
